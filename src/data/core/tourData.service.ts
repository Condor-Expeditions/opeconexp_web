...
import { formatPrice, getValidImageUrl, resolveLabel } from "./utils";
...
export function getTourData(
	...
	const category = tour.categories?.[0]
		? getCategory(tour.categories[0].id)
		: undefined;
	const difficulty = tour.difficulty
		? getDifficulty(tour.difficulty)
		: undefined;
	...
	return {
		...
		category: resolveLabel(
			category?.title || {
				es: tour.categories?.[0]?.id || "otros",
				en: tour.categories?.[0]?.id || "other",
			},
			lang,
		),
		...
		pricing: {
			general: formatPrice(
				tour.prices?.[0]?.amount || 0,
				tour.prices?.[0]?.currency,
			),
			special: formatPrice(
				(tour.prices?.[0]?.amount || 0) * 0.9,
				tour.prices?.[0]?.currency,
			),
		},
	};
