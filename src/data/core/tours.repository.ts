...
import type { Category, Difficulty, Tour } from "./types";
...
export function getTours(filters?: {
	...
	if (filters?.category) {
		result = result.filter((t) =>
			t.categories?.some((c) => c.id === filters.category),
		);
	}
...
export function getCategoryTitle(
	...
	if (typeof categorySlug !== "string") {
		return (
			categorySlug.title?.[lang] ?? categorySlug.title?.es ?? categorySlug.id
		);
	}
...
export function getDifficultyLevel(
	...
	return diff
		? (diff.title[lang] ?? diff.title.es ?? difficultySlug)
		: difficultySlug;
}
